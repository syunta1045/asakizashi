...
1. **1.0.3 を出す（★ASOも一緒に反映）** — §5 の手順（build 18 → ASC 新規1.0.3 → What's New → 提出）。中身は CHANGELOG の [1.0.3]。
   **⚠️ ASO は 1.0.3 提出と一体でしか反映できない**（名前・サブ・カテゴリ・キーワード・スクショは全部バージョン紐づき＝単独保存不可）。正典は `docs/app-store-metadata.json`。反映内容:
   - アプリ情報（バージョン作成後に保存）: 名前=`朝しるべ－朝の気分記録・習慣・日記` ／ サブ=`毎日1分で続く、気分ジャーナルと習慣記録` ／ カテゴリ=ヘルスケア/フィットネス主・ライフスタイル副
   - **⚠️ 旧サブは `毎朝のしるべと占い`＝「占い」が残存**（脱occult漏れ・現行listingに出ている可能性）。新サブで除去される＝**1.0.3を出すほど早く消える**
   - バージョン: キーワード（json の95字・名前/サブと重複させない）／ スクショは `store-screenshots/out/` の新7枚に差し替え
   - H&Fカテゴリで「規制対象の医療用デバイス申告」バナーが出るが本アプリは非該当（2027年初頭までにUS/EU/UK配信時のみ・JPは無関係）
...
3. **集客・ASO の実施状況** — 設計・素材は完了（`docs/app-store-metadata.json`＝名前/サブ/キーワード/カテゴリ、`store-screenshots/out/`＝新スクショ7枚、アプリ内レビュー依頼=`lib/review.ts`実装済み）。ASCへの反映は↑item1（1.0.3）と一体。スクショ画像自体はまだ MBTI/血液型チップが目立つので、シミュレータでMBTI未設定プロフィールで再撮影すると再ポジションに完全一致（任意）。
